...
{
    std::strstream ss;
    ss << "Starting trade with parameters:" << std::endl <<
...
{
    std::strstream ss;
    ss << std::put_time(std::localtime(&timestamp), "%F %T\t") <<
...
{
    std::strstream ss;
    ss << std::put_time(std::localtime(&timestamp), "%F %T\t") << myCurrencySymbol << ": " << std::to_string(myCurrencyBalance) << "\t" <<
